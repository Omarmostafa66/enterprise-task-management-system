...
async function register() {
  ...
  try {
    ...
    /* Registration now creates Employee accounts only */
...
function logout(){
  ...
  if(typeof customConfirm === 'function') {
    ...
    customConfirm(
      "Are you sure you want to log out and end the session?",
      () => {
        executeLogout();
      },
      'btn-d',
...
function applyRoleUI(){
  ...
  if(role==='admin') {
    ...
    chip.classList.add('r-admin');
    chip.textContent='Admin';

  } else if(role==='project_manager') {

    chip.classList.add('r-pm');
    chip.textContent='Proj. Manager';

  } else {

    chip.classList.add('r-employee');
  ...
  if(role==='admin') {
    ...
    rt.textContent='Administrator';
    rs.textContent='Full access granted';

  } else if(role==='project_manager') {

    rt.textContent='Project Manager';
    rs.textContent='Project & task management';

  } else {

    rt.textContent='Employee';
    rs.textContent='Task view & status updates';
  }
...
function openProfile() {
  ...
  let roleFormatted =
    (CURRENT_USER_ROLE || 'employee').replace('_', ' ');
...
async function updateProfile() {
  ...
  btn.innerHTML =
    '<i class="fa-solid fa-circle-notch fa-spin-slow"></i> Saving...';
...
function refreshDash(){
  ...
  if (
    ...
    typeof loadUsersForTaskAssignment === 'function'
  ) {
    promises.push(loadUsersForTaskAssignment());
  }

  /* Update project managers dropdown */
  if (
    CURRENT_USER_ROLE !== 'employee' &&
    typeof loadManagersForProject === 'function'
  ) {
    promises.push(loadManagersForProject());
  }
  ...
  Promise.all(promises)
    .finally(()=>{
      ...
      if(icon) {
        setTimeout(()=>icon.classList.remove('fa-spin-slow'),600);
      }
